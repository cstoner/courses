...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        # I don't act too smart here. 
        # Essentially, I turn Prod(Sum, Sum) into Sum(Prod(Prod, Sum)) and 
        # then let the Prod(Prod, Sum) case below handle further simplification
        result = Sum()
        for item in expr1:
            result.append(Product([item]+expr2))

    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        result = Sum()
        for item in expr1:
            result.append(Product(expr2+[item]))

    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        result = Sum()
        for item in expr2:
            result.append(Product(expr1+[item]))

    elif isinstance(expr1, Product) and isinstance(expr2, Product):
        # Despite what this looks like, we are concatenating lists, not adding numbers
